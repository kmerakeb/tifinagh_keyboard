document.addEventListener("DOMContentLoaded", function () {
 ...
 if (!input || !container) {
  console.log("Missing input or keyboard container");
  return;
}
...
function createRow(keys, addDelete = false, addShift = false) {
...
if (addShift) {
  ...
  shiftBtn.classList.add("key-shift", "wide");


  if (currentLayout === 'kabyle') {
    if (isShift) {
...
let layout;
if (currentLayout === 'tifinagh') {
  layout = tifinaghRows;
} else {
  layout = isShift ? kabyleRowsUpper : kabyleRowsLower;
}
...
layout.forEach((row, index) => {
  ...
  // Only add Shift button on last Kabyle row
  const addShift = currentLayout === 'kabyle' && isLast;

  container.appendChild(createRow(row, isLast, addShift));
});


  // Spacebar + Switch row
// Spacebar + Shift + Switch row
...
const switchBtn = createButton(currentLayout === 'tifinagh' ? "kabyle ⌨️" : "ⵜⵉⴼⵉⵏⴰⵖ ⌨️");
switchBtn.classList.add("key-switch");
switchBtn.addEventListener("click", () => {
  currentLayout = currentLayout === 'tifinagh' ? 'kabyle' : 'tifinagh';
  isShift = false;
  renderKeyboard();
});
